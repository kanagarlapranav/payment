...
def export_database_to_json(output_path: Path = None) -> dict:
    ...
    to a versioned JSON structure (Format v2) and saves to disk.
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            for tx in transactions:
                ...
                if existing:
                    # Update existing record if incoming update is newer or same
                    existing_updated = existing.get('updated_at') or ''
                    # If incoming is newer or equal, apply update
                    db_id = existing['id']
                    cursor.execute('''
            ...
            for dish in custom_menu_items:
                ...
                ''', (dish['name'], float(dish['price']), dish.get('category', 'Snacks & Tea'), int(dish.get('is_veg', 1))))
